...
class CoreImageFieldEdit extends CoreFieldBase {
	...
	buttonNameAdd = this.translate("Add Image"); //"Add Image";
	buttonNameDelete = this.translate("Delete Image"); //"Add Image";
	textAdd = this.translate("Add Image Text");//"Please click on the Button to select a Image";
	titleAddImage = this.translate("Add Image");//"Add Image";
	labelImage = this.translate("Image");//"Image";
	labelPreview = this.translate("Image Preview");//"Preview";
	...
	_initNodes() {
		...
		this._imagefld = this.querySelector('#image');
	...
	_addListener() {
		this._imagefld.addEventListener('change', this._changeEvent.bind(this));
	}
	...
	_renderField(field) {
		...
		<core-select-media data-selectable-types="image" data-fieldid="image" data-api-subtreeitems="/api/core/assetlib/subitems" data-rootid="22222222-2222-2222-2222-222222222222">
			<input type="text" class="control hidden" id="image" name="${field.fieldName}.AssetId" value="${value == null ? "" : value}">
		</core-select-media>
	...
	getFieldValues() {
        ...
        if (this.hasAdditionalFields) {
            var result = [{
				fieldValue: this.querySelector(`#image`).value,
				fieldType: "guid",
...
window.customElements.define('core-imagefield-edit', CoreImageFieldEdit);
