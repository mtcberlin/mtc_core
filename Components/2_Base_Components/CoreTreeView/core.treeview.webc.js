class CoreTreeView extends CoreHTMLElement {
	...
	_propagateState(id) {
		if (this._lastSelectedItemId === "" && this.querySelectorAll("[selected]").length > 0) {
			this.querySelectorAll("[selected]").forEach(ele => {
				const eleId = ele.getAttribute("selected");
				if(eleId != id) {
					this._lastSelectedItemId = eleId;
				}
			});
		}
