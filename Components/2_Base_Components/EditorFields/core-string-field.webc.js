class CoreStringField extends CoreFieldBase {
  _additionalFields = {
    SimpleText: {
      fldtype: "text",
      valuetype: "string",
  ...
  _readInfos() {
    ...
    if (this.hasAttribute("extended")) {
      this.hasAdditionalFields = this.getAttribute("extended") == "true";
    }
  ...
  _renderField(field) {
    ...
    return `
        <div class="js-text c-fld-main mb-3">
            <label class='form-label c-fld-label fw-bold text-dark-gray' for='${
              field.fieldName
            }'>${displayName}</label>
            <input class='form-control js-main-fld me-5' type='text' id='${
              field.fieldName
...
window.customElements.define("core-string-field", CoreStringField);
